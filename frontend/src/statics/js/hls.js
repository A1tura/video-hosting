class Loader extends Hls.DefaultConfig.loader {
    ...
    load(context, config, callbacks) {
        ...
        if (context.type === "fragment" || context.type === "segment" || context.type == undefined) {
            ...
            context.url = `http://127.0.0.1:8081/segment/${url[url.length - 1]}/${segmentPath.split(".")[0]}`;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (Hls.isSupported()) {
        ...
        const url = window.location.href.split("/")
        hls.loadSource('http://127.0.0.1:8081/video/' + url[url.length - 1]); // HLS playlist that references .ts files
